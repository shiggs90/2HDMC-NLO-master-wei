...
//   printf("input type,tb,cba\n");
  ...
  for(double tbexp  = -2; tbexp<=0.7; tbexp=tbexp+0.03)
  {
      // double tb  = pow(10,tbexp);
      double tb=0.5;
      for(double cba  = -0.8; cba<=0.8; cba=cba+0.003)
      {
          double mh  = 125;
          double mC;
          cba=0;
         ...
         // 
          ...
          name2<< mH;
          name3<< mA;
          string oputname="Brtbcba_"+name1.str()+name2.str()+name3.str()+"_ana.txt";
          ...
          brout <<" "<<setw(12)<<_gammatot[0]<<" "<<setw(12)<<_gammatot[1]<<" "<<setw(12)<<_gammatot[2]<<endl;
