...
// this one is based on 0813-version for detail study
...
//   printf("input type,tb,cba\n");
  ...
  for(double tbexp  = -2; tbexp<=1.7; tbexp=tbexp+0.003)
  {
      double tb  = pow(10,tbexp);
      // double tb=0.5;
      for(double cba  = -0.99; cba<=0; cba=cba+0.001)
      {
          double mh  = 125;
          double mC;
          // cba=0;
         ...
         //
          ...
          name2<< mH;
          name3<< mA;
          string oputname="./work/output_tbcba_0817/Brtbcba_"+name1.str()+name2.str()+name3.str()+"L.txt";
          ...
          brout <<" "<<setw(12)<<_gammatot[1]<<" "<<setw(12)<<_gammatot[2]<<endl;
