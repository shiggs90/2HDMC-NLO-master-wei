...
double DecayTable::get_gamma_hdd(int h, int d1, int d2) {
  ...
  complex <double> cs,cp;
#ifdef NLOCOUPLINGS
  model.get_coupling_hdd_NLO(h,d1,d2,cs,cp);
#else
...
double DecayTable::get_gamma_huu(int h, int u1, int u2) {
  ...
  if (M < (m1+m2))
  {
      model.get_coupling_huu(h,u1,u2,cs,cp); //Off-shell, using LO terms. Further effort is needed for NLO
  }
  else
  {
      model.get_coupling_huu_NLO(h,u1,u2,cs,cp);
  }
#else
...
double DecayTable::get_gamma_hll(int h, int l1, int l2) {
  ...
  complex <double> cs,cp;
#ifdef NLOCOUPLINGS
  model.get_coupling_hll_NLO(h,l1,l2,cs,cp);
#else
...
double DecayTable::get_gamma_hgg(int h) {
  ...
  gamma_hgg[h] = hgg(h);
  // cout<<"hgg(h) = "<<h<<" "<<hgg(h)<<endl;
  return gamma_hgg[h];
}

double DecayTable::get_gamma_hgg_NLO(int h) {

  if (!qcd_on) return 0.;

  if ((h<1)||(h>3)) return 0.;

  // cout<<"gamma_hgg_NLO[h] @DecayTable.cpp Before cal = "<<h<<" "<<gamma_hgg_NLO[h]<<endl;

  if (gamma_hgg_NLO[h]>=0) return gamma_hgg_NLO[h];
  
  gamma_hgg_NLO[h] = hgg_NLO(h);
  // cout<<"hgg_NLO(h) = "<<h<<" "<<hgg_NLO(h)<<endl;
  return gamma_hgg_NLO[h];
}
...
double DecayTable::get_gamma_hhh(int h, int h1, int h2) {
  ...
  {
    model.get_coupling_hhh_NLO(h,h1,h2,c);
  }
#else
  model.get_coupling_hhh(h,h1,h2,c);
#endif
...
double DecayTable::get_gammatot_h(int h) {
  ...
  // cout<<" gammatot_h[1] @ DecayTable.cpp " << gammatot_h[1]<<endl;
  // cout<<"table.get_gamma_huu(1,3,3)@ DecayTable.cpp = " <<get_gamma_huu(1,3,3)<<endl;
...
double DecayTable::hvv_all(int h, int V,double M) {
  ...
  // cout<<" KHVV_NLO @DecayTable.cpp = "<<KHVV<<endl;
#else
...
double DecayTable::hvh_onshell(int H, int V, int h, double M) {
  ...
  complex <double> c;
#ifdef NLOCOUPLINGS
  model.get_coupling_vhh_NLO(V,H,h,c);
#else
  model.get_coupling_vhh(V,H,h,c);
#endif
...
double DecayTable::hgg_NLO(int h) {
  ...
  for (int i=1;i<=3;i++) {
    ...
    double m = sm.get_dmass_MSbar(i);
    double mp = sm.get_dmass_pole(i);  
    model.get_coupling_hdd_NLO(h,i,i,gS,gP);  
  ...
  for (int i=1;i<=3;i++) {
    ...
    {
        model.get_coupling_huu_NLO(h,i,i,gS,gP);
    }    
